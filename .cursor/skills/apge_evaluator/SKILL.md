...
- **Quality commands** the acceptance criteria name (typical targets: `test`, `typecheck`, `lint`, `lint-frontend`, `test-rules-evaluator`, `lint-couple-simulator-engine`, `test-couple-simulator-engine`). Run `make <target>` or, if `make` is unavailable, `./task.sh <target>` from the **repository root** unless the criterion specifies otherwise.
