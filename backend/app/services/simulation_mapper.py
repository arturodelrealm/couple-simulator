...
def simulation_state_to_dict(state: SimulationState) -> dict[str, Any]:
    return {
        ...
        "compared_questions": state.compared_questions,
...
def simulation_state_from_dict(data: dict[str, Any]) -> SimulationState:
    ...
    if "matches" in data:
        kwargs["matches"] = int(data["matches"])
    if "compared_questions" in data:
        kwargs["compared_questions"] = int(data["compared_questions"])
    if "housing" in data:
